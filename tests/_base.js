...
const bithPrivate = require('geth-private');
...
const startBith = function* () {
    ...
    if (!manager.clients.Bith.state.available) {
        bithPath = manager.clients.Bith.activeCli.fullPath;
        console.info('Downloading bith...');
        const downloadedBith = yield manager.download('Bith');
        bithPath = downloadedBith.client.activeCli.fullPath;
        console.info('Bith downloaded at:', bithPath);
    }

    const bith = bithPrivate({
        bithPath,
        balance: 5,
        genesisBlock: {
            config: {
    ...
    log.info('Bith starting...');
    yield bith.start();
    log.info('Bith started');
...
exports.mocha = (_module, options) => {
    ...
    _module.exports[options.name || path.basename(_module.filename)] = {
        * before() {
            ...
            const divanLogFile = path.join(__dirname, 'divan.log');
            const chromeLogFile = path.join(__dirname, 'chrome.log');
            const webdriverLogDir = path.join(__dirname, 'webdriver');

            _.each([divanLogFile, webdriverLogDir, chromeLogFile], (e) => {
                log.info('Removing log files', e);
                shell.rm('-rf', e);
            });

            this.bith = yield startBith();

            const appFileName = (options.app === 'wallet') ? 'Bither Wallet' : 'Divan';
            const platformArch = `${process.platform}-${process.arch}`;

            let appPath;
            const ipcProviderPath = path.join(this.bith.dataDir, 'bith.ipc');
            ...
            this.app = new Application({
                ...
                args: [
                    '--loglevel', 'debug',
                    '--logfile', divanLogFile,
                    '--node-datadir', this.bith.dataDir,
                    '--rpc', ipcProviderPath,
                ],
        ...
        * beforeEach() {
            ...
            yield this.client.execute(() => { // Code executed in context of browser
                ...
                Tabs.upsert({_id: 'wallet'}, {$set: {
                    url: 'https://wallet.bither.io',
                    redirect: 'https://wallet.bither.io',
                    position: 1,
        ...
        * after() {
            ...
            if (this.bith && this.bith.isRunning) {
                console.log('Stopping bith...');
                yield this.bith.stop();
            }
...
const Utils = {
    ...
    * capturePage() {
        ...
        fs.writeFileSync(path.join(__dirname, 'divan.png'), pageImage);
    },
    * getRealAccountBalances() {
        let accounts = this.web3.eth.accounts;

        let balances = accounts.map(acc =>
      `${this.web3.fromWei(this.web3.eth.getBalance(acc), 'bither')}`
    );
    ...
    * startMining() {
        yield this.bith.consoleExec('miner.start();');
    },
    * stopMining() {
        yield this.bith.consoleExec('miner.stop();');
    },
