...
test['Check for Divan title'] = function* () {
    (yield this.client.getTitle()).should.eql('Divan');
};
...
test['Browser bar should not execute JS'] = function* () { // ETH-01-001
    ...
    const divan = yield client.execute(() => { return window.divan; }); // checking if `execute` works
    const pwned = yield client.execute(() => { return window.pwned; });

    should.exist(divan.value);
...
//test['Links with target _blank should open inside Divan'] = function* () {
//    const client = this.client;
...
// test['Links with target _popup should open inside Divan'] = function* () {
//     const client = this.client;
//     yield this.navigateTo(`${this.fixtureBaseUrl}/fixture-popup.html`);
//     yield this.getWindowByUrl(e => /fixture-popup.html$/.test(e));
//
...
// ETH-01-005
// test['Divan main webview should not redirect to arbitrary addresses'] = function* () {
//     const client = this.client;
...
// ETH-01-008
test['Divan main webview should not redirect to local files'] = function* () {
    const client = this.client;
