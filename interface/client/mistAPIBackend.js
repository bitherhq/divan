...
/**
The backend side of the divan API.

@method divanAPIBackend
*/
divanAPIBackend = function (event) {
    var template = this.template;
    var webview = this.webview;
    var arg = event.args[0];

    // console.trace('divanAPIBackend event', event);
    ...
    if (event.channel === 'divanAPI_sound') {
        sound.pause();
    ...
    if (event.channel === 'divanAPI_setBadge') {
        Tabs.update(template.data._id, { $set: {
            badge: arg,
        } });
    }

    if (event.channel === 'divanAPI_menuChanges' && arg instanceof Array) {
