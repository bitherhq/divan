...
Template['popupWindows_splashScreen'].onCreated(function () {
    ...
    ipc.on('uiAction_clientBinaryStatus', function (e, status) {
        TemplateVar.set(template, 'text', TAPi18n.__('divan.startScreen.clientBinaries.' + status));
    ...
    ipc.on('uiAction_nodeStatus', function (e, status, errorTag) {
        switch (status) {
        case 'starting':
            TemplateVar.set(template, 'text', TAPi18n.__('divan.startScreen.nodeStarting'));
            showNodeLog = true;
            TemplateVar.set(template, 'logText', null);
            TemplateVar.set(template, 'showProgressBar', false);
            TemplateVar.set(template, 'showStartAppButton', false);
        ...
        case 'started':
            TemplateVar.set(template, 'text', TAPi18n.__('divan.startScreen.nodeStarted'));
            break;

        case 'connected':
            TemplateVar.set(template, 'text', TAPi18n.__('divan.startScreen.nodeConnected'));
            lastSyncData = {};
            break;

        case 'stopping':
            TemplateVar.set(template, 'text', TAPi18n.__('divan.startScreen.nodeStopping'));
            TemplateVar.set(template, 'showProgressBar', false);
            TemplateVar.set(template, 'showStartAppButton', false);
            break;

        case 'stopped':
            TemplateVar.set(template, 'text', TAPi18n.__('divan.startScreen.nodeStopped'));
            break;

        case 'connectionTimeout':
            TemplateVar.set(template, 'text', TAPi18n.__('divan.startScreen.nodeConnectionTimeout'));
            break;

        case 'error':
            errorTag = 'divan.startScreen.' + (errorTag || 'nodeError');
    ...
    ipc.on('uiAction_nodeSyncStatus', function (e, status, data) {
        ...
        if (status === 'inProgress') {
            ...
            TemplateVar.set(template, 'startAppButtonText', TAPi18n.__('divan.startScreen.launchApp'));
            ...
            if (data !== false) {
                ...
                if (web3.net.peerCount > 0) {
                    ...
                    if (0 < lastSyncData._displayKnownStates && (
                        ...
                        translationString = 'divan.startScreen.nodeSyncInfoStates';
                    } else {
                        // Mostly downloading blocks
                        translationString = 'divan.startScreen.nodeSyncInfo';
                    }
                } else {
                    // Not online
                    translationString = 'divan.startScreen.nodeSyncConnecting';
                }
                ...
                if (web3.net.peerCount > 1) {
                    translationString = 'divan.startScreen.nodeSyncFoundPeers';
                } else {
                    translationString = 'divan.startScreen.nodeSyncConnecting';
                }
            ...
            TemplateVar.set(template, 'text', TAPi18n.__('divan.startScreen.nodeSyncing'));
...
Template['popupWindows_splashScreen'].helpers({
    ...
    */
    'iconPath': function () {
        return 'file://' + window.dirname + '/icons/' + window.divanMode + '/icon2x.png';
    },
    /**
