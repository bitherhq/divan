...
Template['popupWindows_sendTransactionConfirmation'].helpers({
    ...
    */
    'totalAmount': function () {
        var amount = EthTools.formatBalance(this.value, '0,0.00[0000000000000000]', 'bither');
    ...
    /**
    Calculates the fee used for this transaction in bither
    ...
    */
    'estimatedFee': function () {
        ...
        if (gas && this.gasPrice) {
            return EthTools.formatBalance(new BigNumber(gas, 10).times(new BigNumber(this.gasPrice, 10)), '0,0.0[0000000] unit', 'bither');
        }
    },
    /**
    Calculates the provided gas amount in bither
    ...
    */
    'providedGas': function () {
        ...
        if (gas && this.gasPrice) {
            return EthTools.formatBalance(new BigNumber(gas, 10).times(new BigNumber(this.gasPrice, 10)), '0,0.0[0000000]', 'bither');
        }
    },
    /**
...
Template['popupWindows_sendTransactionConfirmation'].events({
    /**
    Gets the new provided gas in bither amount and calculates the resulting providedGas
    ...
    'submit form': function (e, template) {
        ...
        if (data.to && data.from === data.to.toLowerCase()) {
            GlobalNotification.warning({
                content: TAPi18n.__('divan.popupWindows.sendTransactionConfirmation.errors.sameAccount'),
                duration: 5
            });
        ...
        web3.personal.sendTransaction(data, pw || '', function (e, res) {
            ...
            if (!e && res) {
                ...
                if (e.message.indexOf('Unable to connect to socket: timeout') !== -1) {
                    GlobalNotification.warning({
                        content: TAPi18n.__('divan.popupWindows.sendTransactionConfirmation.errors.connectionTimeout'),
                        duration: 5
                    });
                } else if (e.message.indexOf('could not decrypt key with given passphrase') !== -1) {
                    GlobalNotification.warning({
                        content: TAPi18n.__('divan.popupWindows.sendTransactionConfirmation.errors.wrongPassword'),
                        duration: 3
                    });
                } else if (e.message.indexOf('multiple keys match address') !== -1) {
                    GlobalNotification.warning({
                        content: TAPi18n.__('divan.popupWindows.sendTransactionConfirmation.errors.multipleKeysMatchAddress'),
                        duration: 10
                    });
                } else if (e.message.indexOf('Insufficient funds for gas * price + value') !== -1) {
                    GlobalNotification.warning({
                        content: TAPi18n.__('divan.popupWindows.sendTransactionConfirmation.errors.insufficientFundsForGas'),
                        duration: 5
                    });
                } else {
                    GlobalNotification.warning({
