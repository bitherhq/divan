        ...
        template._intervalId = Meteor.setInterval(function () {
            ...
            if (syncing) {
                ...
                if (Math.round(1000 * Number(syncing._displayStatesDownload)) !== Math.round(1000 * Number(syncing.pulledStates / (syncing.knownStates + 1)))) {
                    TemplateVar.set(template, 'syncStatusMessageLive', TAPi18n.__('divan.popupWindows.onboarding.syncMessageWithStates', syncing));
                } else if (syncing.displayBlock === '0') {
                    TemplateVar.set(template, 'syncStatusMessageLive', '');
                } else {
                    TemplateVar.set(template, 'syncStatusMessageLive', TAPi18n.__('divan.popupWindows.onboarding.syncMessage', syncing));
                }
            }
...
Template['popupWindows_onboardingScreen_importAccount'].events({
            ...
            ipc.on('uiAction_checkedWalletFile', function (ev, error, type) {
                ...
                default:
                    GlobalNotification.warning({
                        content: TAPi18n.__('divan.popupWindows.onboarding.errors.unknownFile'),
                        duration: 4
                    });
                }
            });
    ...
    */
    ...
    /**
    Checks the password match sends the file path and password to the divan backend to import
        ...
        ipc.on('uiAction_importedWalletFile', function (ev, error, address) {
            ...
            if (address) {
                ...
                if (error === 'Decryption Failed') {
                    GlobalNotification.warning({
                        content: TAPi18n.__('divan.popupWindows.onboarding.errors.wrongPassword'),
                        duration: 4
                    });
                } else {
                    GlobalNotification.warning({
                        content: TAPi18n.__('divan.popupWindows.onboarding.errors.importFailed', { error
                        }),
                        duration: 4
                    });
                }
...
Template['popupWindows_onboardingScreen_password'].events({
    ...
    */
    'submit form': function (e, template) {
        ...
        if (pw !== pwRepeat) {
            GlobalNotification.warning({
                content: TAPi18n.__('divan.popupWindows.requestAccount.errors.passwordMismatch'),
                duration: 3
            });
        } else if (pw && pw.length < 8) {
            GlobalNotification.warning({
                content: TAPi18n.__('divan.popupWindows.requestAccount.errors.passwordTooShort'),
                duration: 3
            });
        } else if (pw && pw.length >= 8) {
            ...
            web3.personal.newAccount(pw, function (e, res) {
                ...
                alert(TAPi18n.__('divan.popupWindows.requestAccount.backupHint'));
                ...
                if (!e) {
                    ...
                    GlobalNotification.warning({
                        content: TAPi18n.__('divan.popupWindows.onboarding.errors.nodeNotStartedYet'),
                        duration: 4
                    });
                }
            });
