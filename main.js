...
const bitherNode = require('./modules/bitherNode.js');
...
// - DIVAN
    ...
    log.info('Starting in Divan mode');
...
WARNING: You are connecting to an Bither node via: ${Settings.rpcHttpPath}
...
onReady = () => {
        ...
        mainWindow = Windows.create('main', {
            ...
            electronOptions: {
                ...
                webPreferences: {
                    ...
                    preload: `${__dirname}/modules/preloader/divanUI.js`,
        ...
        timesync.checkEnabled((err, enabled) => {
            ...
            if (!enabled) {
                dialog.showMessageBox({
                    type: 'warning',
                    buttons: ['OK'],
                    message: global.i18n.t('divan.errors.timeSync.title'),
                    detail: `${global.i18n.t('divan.errors.timeSync.description')}\n\n${global.i18n.t(`divan.errors.timeSync.${process.platform}`)}`,
                }, () => {
                });
            }
        });
    ...
    const kickStart = () => {
        ...
        Q.try(() => {
            ...
            if ((Settings.loadUserData('daoFork') || '').trim() === 'false') {
                dialog.showMessageBox({
                    type: 'warning',
                    buttons: ['OK'],
                    message: global.i18n.t('divan.errors.legacyChain.title'),
                    detail: global.i18n.t('divan.errors.legacyChain.description')
                }, () => {
                    shell.openExternal('https://github.com/bitherhq/divan/releases');
                    app.quit();
                });
        ...
        .then(function sanityCheck() {
            if (!bitherNode.isIpcConnected) {
                throw new Error('Either the node didn\'t start or IPC socket failed to connect.');
            }

            /* At this point Bith is running and the socket is connected. */
        ...
        .then(function getAccounts() {
            return bitherNode.send('eth_accounts', []);
        })
        .then(function onboarding(resultData) {

            if (bitherNode.isBith && (resultData.result === null || (_.isArray(resultData.result) && resultData.result.length === 0))) {
