...
window.addEventListener('message', function message(event) {
    ...
    // BitherProvider: connect
    if (data.type === 'create') {
        ipcRenderer.send('ipcProvider-create');

    // BitherProvider: write
    ...
    // divanAPI
    } else if (/^divanAPI_[a-z]/i.test(data.type)) {

        if (data.type === 'divanAPI_requestAccount') {
...
// custom Events
['uiAction_windowMessage', 'divanAPI_callMenuFunction'].forEach(function (type) {
...
let divanAPI = fs.readFileSync(path.join(__dirname, '/injected/divanAPI.js')).toString();
const bitherProvider = fs.readFileSync(path.join(__dirname, '/injected/BitherProvider.js')).toString();

divanAPI = divanAPI.replace('__version__', packageJson.version)
