...
module.exports = () => {
    ...
    Provides an API for all dapps, which specifically targets features from the Divan browser
    ...
    */
    const divan = {
        ...
        requestAccount(callback) {
            ...
            ipcRenderer.send('divanAPI_requestAccount');
        },
        solidity: {
        ...
        sounds: {
            bip: function playSound() {
                ipcRenderer.sendToHost('divanAPI_sound', `file://${__dirname}/../../../sounds/bip.mp3`);
            },
            bloop: function playSound() {
                ipcRenderer.sendToHost('divanAPI_sound', `file://${__dirname}/../../../sounds/bloop.mp3`);
            },
            invite: function playSound() {
                ipcRenderer.sendToHost('divanAPI_sound', `file://${__dirname}/../../../sounds/invite.mp3`);
            },
        },
        menu: {
            entries: {},
            ...
            Example
                ...
                divan.menu.setBadge('Some Text')

            @method setBadge
            @param {String} text
            */
            setBadge(text) {
                ipcRenderer.sendToHost('divanAPI_setBadge', text);
            },
            /**
            ...
            */
            add(id, options, callback) {
                ...
                if (!(filteredId in this.entries) &&
                    Object.keys(this.entries).length >= DIVAN_SUBMENU_LIMIT) {
                    return false;
                }

                const entry = {
                    id: filteredId || 'divan_defaultId',
            ...
            /**
            Updates a menu entry from the divan sidebar.
            ...
            */
            ...
            /**
            Removes a menu entry from the divan sidebar.
    ...
    ipcRenderer.on('divanAPI_callMenuFunction', (e, id) => {
        if (divan.menu.entries[id] && divan.menu.entries[id].callback) {
            divan.menu.entries[id].callback();
        }
    });

    ipcRenderer.on('uiAction_windowMessage', (e, type, error, value) => {
        console.log('uiAction_windowMessage',type, error, value);
        if (divan.callbacks[type]) {
            divan.callbacks[type].forEach((cb) => {
                cb(error, value);
            });
            delete divan.callbacks[type];
        }
    });
    ...
    setInterval(() => {
        if (queue.length > 0) {
            ipcRenderer.sendToHost('divanAPI_menuChanges', queue);
            queue = [];
        }
    }, 500);
