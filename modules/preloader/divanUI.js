/**
@module preloader DivanUI
*/

require('./include/common')('divan');
...
menu.append(new MenuItem({ label: i18n.t('divan.rightClick.reload'), accelerator: 'Command+R', click() {
    const webview = Helpers.getWebview(LocalStore.get('selectedTab'));
    if (webview) {
        webview.reloadIgnoringCache();
    }
} }));
menu.append(new MenuItem({ label: i18n.t('divan.rightClick.openDevTools'), click() {
    const webview = Helpers.getWebview(LocalStore.get('selectedTab'));
    if (webview) {
        webview.openDevTools();
    }
} }));
menu.append(new MenuItem({ label: i18n.t('divan.rightClick.inspectElements'), click() {
    const webview = Helpers.getWebview(LocalStore.get('selectedTab'));
    if (webview) {
