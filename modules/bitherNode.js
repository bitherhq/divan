...
/**
 * Bither nodes manager.
 */
class BitherNode extends EventEmitter {
    constructor() {
        super();
    ...
    /**
     * Start an bither node.
     * @param  {String} nodeType bith, eth, etc
    ...
    _start(nodeType, network, syncMode) {
        ...
        return this.stop()
            ...
            .catch((err) => {
                ...
                // if unable to start eth node then write bith to defaults
                if (nodeType === 'eth') {
                    Settings.saveUserData('node', 'bith');
                }
            ...
            logRotate(Settings.constructUserDataPath('node.log'), { count: 5 }, (err) => {
                    ...
                    if (STATES.STARTING === this.state) {
                        ...
                        if (nodeType === 'bith') {
                            if (dataStr.indexOf('fatal: error') >= 0) {
                                const error = new Error(`Bith error: ${dataStr}`);
                ...
                this.once('data', () => {
                    /*
                        ...
                        errors, etc (see bith port-binding error above)
                    */
                    setTimeout(() => {
                        if (STATES.STARTING === this.state) {
...
BitherNode.STARTING = 0;


module.exports = new BitherNode();
