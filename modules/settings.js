...
const argv = require('yargs')
    .usage('Usage: $0 [Divan options] [Node options]')
    .option({
        mode: {
            alias: 'm',
            demand: false,
            default: defaultConfig.mode,
            describe: 'App UI mode: wallet, divan.',
            requiresArg: true,
            nargs: 1,
            type: 'string',
            group: 'Divan options:',
        ...
        swarmurl: {
            ...
            describe: 'URL serving the Swarm HTTP API. If null, Divan will open a local node.',
            requiresArg: true,
            nargs: 1,
            type: 'string',
            group: 'Divan options:',
        ...
        bithpath: {
            demand: false,
            describe: 'Path to Bith executable to use instead of default.',
            requiresArg: true,
            nargs: 1,
            type: 'string',
            group: 'Divan options:',
        ...
        'reset-tabs': {
            demand: false,
            describe: 'Reset Divan tabs to their default settings.',
            requiresArg: false,
            nargs: 0,
            type: 'boolean',
            group: 'Divan options:',
        ...
        syncmode: {
            demand: false,
            requiresArg: true,
            describe: 'Bith synchronization mode: [fast|light|full]',
            nargs: 1,
            type: 'string',
            group: 'Divan options:',
        },
        version: {
            ...
            requiresArg: false,
            nargs: 0,
            describe: 'Display Divan version.',
            group: 'Divan options:',
            type: 'boolean',
        },
        skiptimesynccheck: {
        ...
        '': {
            describe: 'To pass options to the underlying node (e.g. Bith) use the --node- prefix, e.g. --node-datadir',
            group: 'Node options:',
        },
    })
    .help('h')
...
class Settings {
    ...
    get userDataPath() {
    // Application Aupport/Divan
        return app.getPath('userData');
    }

    get dbFilePath() {
        const dbFileName = (this.inAutoTestMode) ? 'divan.test.lokidb' : 'divan.lokidb';
    ...
    get appName() {
        return this.uiMode === 'divan' ? 'Divan' : 'Bither Wallet';
    }
    ...
    get rpcMode() {
        ...
        if (argv.rpc && argv.rpc.indexOf('ws:') === 0) {
            this._log.warn('Websockets are not yet supported by Divan, using default IPC connection');
            argv.rpc = null;
            return 'ipc';
        } else
            return 'ipc';
    ...
    get rpcIpcPath() {
        ...
        if (process.platform === 'darwin') {
            ipcPath += '/Library/Bither/bith.ipc';
        } else if (process.platform === 'freebsd' ||
       process.platform === 'linux' ||
       process.platform === 'sunos') {
            ipcPath += '/.bither/bith.ipc';
        } else if (process.platform === 'win32') {
            ipcPath = '\\\\.\\pipe\\bith.ipc';
        }
