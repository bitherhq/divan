...
ipc.on('backendAction_checkWalletFile', (e, path) => {
    fs.readFile(path, 'utf8', (event, data) => {
        try {
            ...
            /** result
            *  [ 'bithersale', undefined ]   Bithersale keyfile
            ...
            */
            ...
            if (type === 'bithersale') {
                ...
                // bith
                    ...
                    if (process.platform === 'darwin') keystorePath += '/Library/Bither/keystore';

                    if (process.platform === 'freebsd' ||
                        process.platform === 'linux' ||
                        process.platform === 'sunos') keystorePath += '/.bither/keystore';

                    if (process.platform === 'win32') keystorePath = `${Settings.appDataPath}\\Bither\\keystore`;
                }
...
ipc.on('backendAction_importWalletFile', (e, path, pw) => {
    ...
    const binPath = ClientBinaryManager.getClient('bith').binPath;
    const nodeProcess = spawn(binPath, ['wallet', 'import', path]);

    nodeProcess.once('error', () => {
        error = true;
        e.sender.send('uiAction_importedWalletFile', 'Couldn\'t start the "bith wallet import <file.json>" process.');
    });
...
// DIVAN API
ipc.on('divanAPI_createAccount', createAccountPopup);

ipc.on('divanAPI_requestAccount', (e) => {
