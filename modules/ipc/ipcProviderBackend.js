...
/**
The IPC provider backend filter and tunnel all incoming request to the bither node.
...
const bitherNode = require('../bitherNode');
...
class IpcProviderBackend {
    constructor() {
        ...
        bitherNode.on('state', _.bind(this._onNodeStateChanged, this));
            ...
            if (!socket.isConnected) {
                ...
                if (!this._connectionPromise[ownerId]) {
                    this._connectionPromise[ownerId] = Q.try(() => {
                        ...
                        if (bitherNode.state !== bitherNode.STATES.CONNECTED) {
                            return new Q((resolve, reject) => {
                                const onStateChange = (newState) => {
                                    if (bitherNode.STATES.CONNECTED === newState) {
                                        bitherNode.removeListener('state', onStateChange);

                                        log.debug(`Bither node connected, resume connecting socket ${ownerId}`);

                                        resolve();
                                    }
                                };

                                bitherNode.on('state', onStateChange);
                            });
                        }
                    })
                    .then(() => {
    ...
    /**
     * Handler for when Bither node state changes.
     *
     * Auto-reconnect sockets when bither node state changes
     *
    ...
    _onNodeStateChanged(state) {
        ...
        case bitherNode.STATES.STOPPING:
            log.info('Bither node stopping, disconnecting sockets');
