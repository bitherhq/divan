...
class NodeSync extends EventEmitter {
    ...
    start() {
        ...
        this._syncPromise = Q.try(() => {
            if (!bitherNode.isIpcConnected) {
                throw new Error('Cannot sync - Bither node not yet connected');
            }
            ...
            bitherNode.send('eth_syncing', [])
                .then((ret) => {
                    ...
                    if (result) {
                        ...
                        return bitherNode.send('eth_getBlockByNumber', ['latest', false])
                            .then((ret2) => {
    ...
    _onNodeStateChanged(state) {
        ...
        case bitherNode.STATES.STOPPING:
            log.info('Bither node stopping, so stop sync');

            this.stop();
            break;
            // auto-sync whenever node gets connected
        case bitherNode.STATES.CONNECTED:
            log.info('Bither node connected, re-start sync');
