...
const applicationName = (options.wallet) ? 'Bither Wallet' : 'Divan';
    ...
    if (type === 'wallet') {
        if (options.walletSource === 'local') {
            ...
            bundle(`&& cd ../../meteor-dapp-wallet/app \
                && meteor-build-client ../../divan/dist_${type}/app/interface/wallet -p ""`);
        } else {
            console.log(`Pulling https://github.com/bitherhq/meteor-dapp-wallet/tree/${options.walletSource} "${options.walletSource}" branch...`);
            bundle(`&& cd ../dist_${type} \
                && git clone --depth 1 https://github.com/bitherhq/meteor-dapp-wallet.git \
...
gulp.task('build-dist', (cb) => {
    const appPackageJson = _.extend({}, require('../package.json'), {  // eslint-disable-line global-require
        ...
        homepage: 'https://github.com/bitherhq/divan',
        build: {
            appId: `com.bither.${type}`,
            asar: true,
            directories: {
